...
Default template used when creating new test cases.

**Module:** `plaintest.template`
...
The template uses Python's `string.Template` with the following variables:

- `$title` - Replaced with the capitalized test case title
...
content = TEMPLATE.safe_substitute(title="User login")
print(content)
...
plaintest uses type hints throughout. Import types from standard library:

```python
from pathlib import Path
from typing import Dict, List, Set

def my_function(test_dir: Path) -> Dict[str, List[str]]:
...
Raised by CLI commands to exit with a specific code.

**`FileNotFoundError`**

Raised when test cases directory doesn't exist.
...
Combine plaintest with pytest-html:
...
Track code coverage alongside test case coverage:
...
Use with Allure for comprehensive reporting:
